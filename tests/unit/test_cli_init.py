...
def test_init_postgres_backend(tmp_path: Path) -> None:
    runner = CliRunner()
    # 15 prompts: backend → "postgres" triggers the Postgres URL prompt
    # (adds 1) and skips the SQLite-path prompt the SQLite branch added.
...
def test_init_does_not_offer_cookie_transport(tmp_path: Path) -> None:
    """Regression: the wizard once offered ``"cookie"`` as a transport option
    even though ``TokenTransport`` was narrowed to ``Literal["bearer"]``. An
    operator picking "cookie" got a `regstack.toml` that failed Pydantic
    validation on the very next ``RegStackConfig.load()``. The wizard must no
    longer accept "cookie", and the rendered TOML must not contain a
    ``transport`` or ``cookie_domain`` line at all (the default and the
    deleted-knob, respectively).
    """
    runner = CliRunner()
    result = runner.invoke(
    ...
    # No transport or cookie_domain lines in the generated TOML.
    cfg_text = (tmp_path / "regstack.toml").read_text()
    assert "transport" not in cfg_text
    assert "cookie_domain" not in cfg_text
...
def test_init_smtp_backend_records_smtp_settings(tmp_path: Path) -> None:
    ...
    # Sequence: app/base/proxy (3 defaults), backend default (sqlite),
    # sqlite path default, jwt/jwt-ttl (2 defaults),
